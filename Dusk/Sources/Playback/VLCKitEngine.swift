...
/// PlaybackEngine implementation backed by the older MobileVLCKit 3.x binary.
///
/// This fallback keeps the app working until the PiP-capable VLCKit build is
/// materialized into Frameworks by the install script.
@MainActor
@Observable
final class VLCKitEngine: NSObject, PlaybackEngine {
    private(set) var state: PlaybackState = .idle
    ...
    func makePlayerView() -> AnyView {
        AnyView(LegacyVLCPlayerRepresentable(playerView: vlcView))
    }
    ...
    private func refreshTracks() {
        ...
        if audioCount > 1 {
            availableAudioTracks = (1..<audioCount).map { index in
                AudioTrack(
                    id: audioIndexes[index].intValue,
                    displayTitle: audioNames[index],
                    language: nil,
                    languageCode: nil,
                    codec: nil,
                    channels: nil,
        ...
        selectedAudioTrackID = availableAudioTracks.first(where: {
            $0.id == Int(mediaPlayer.currentAudioTrackIndex)
        })?.id

        let subtitleNames = mediaPlayer.videoSubTitlesNames as? [String] ?? []
        let subtitleIndexes = mediaPlayer.videoSubTitlesIndexes as? [NSNumber] ?? []
        let subtitleCount = min(subtitleNames.count, subtitleIndexes.count)

        if subtitleCount > 1 {
            availableSubtitleTracks = (1..<subtitleCount).map { index in
                SubtitleTrack(
                    id: subtitleIndexes[index].intValue,
                    displayTitle: subtitleNames[index],
                    language: nil,
                    languageCode: nil,
                    codec: nil,
                    isForced: false,
        ...
        selectedSubtitleTrackID = availableSubtitleTracks.first(where: {
            $0.id == currentSubtitleIndex
        })?.id
    }
}
...
private struct LegacyVLCPlayerRepresentable: UIViewRepresentable {
    let playerView: UIView
